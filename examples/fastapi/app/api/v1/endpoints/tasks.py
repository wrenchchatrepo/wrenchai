...
async def create_task(
    ...
    Verifies that the specified agent exists and belongs to the authenticated user before creating the task. Returns the created task data on success, or raises an HTTP 404 error if the agent is not found.
    """
    try:
...
async def list_tasks(
    ...
    Returns:
        A response model containing a list of task data for the user's agents, ordered by creation date descending.
    """
    try:
...
async def get_task(
    ...
    """
    Retrieves a specific task by its UUID for the current authenticated user.
...
async def update_task(
    ...
    Retrieves the specified task by UUID, applies the provided update fields, and returns the updated task data. Raises a 404 error if the task is not found or does not belong to the user.
    """
    try:
        # Query task
        result = await db.execute(
...
async def delete_task(
    ...
    """
    Deletes a task by its UUID if it belongs to an agent owned by the current user.
    
    Raises:
        HTTPException: If the task is not found or an internal error occurs.
    
    Returns:
        ResponseModel[bool]: Success response with True if the task was deleted.
    """
    try:
        # Query task
        result = await db.execute(

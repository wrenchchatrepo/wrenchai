...
def validate_playbook_consistency(playbook: Playbook) -> Tuple[bool, Optional[str]]:
    """
    Checks that all step dependencies, agent references, and tool references in a playbook are valid.
    
    Runs internal consistency checks on the provided Playbook object, returning the first encountered error or confirming validity.
    
    Returns:
        A tuple (is_valid, error_message), where is_valid is True if all checks pass, otherwise False with an error message.
    """

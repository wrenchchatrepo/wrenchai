...
class ToolAuthorizationSystem:
    ...
    def __init__(self, config_path: Optional[str] = None):
        """
        Initializes the tool authorization system, loading configuration and setting up internal mappings.
        
        If a configuration path is provided, it loads tool authorization settings from that file; otherwise, it uses a default path. Initializes permission mappings and usage history for managing tool access and tracking.
        """
    ...
    def _load_configuration(self):
        """
        Loads the tool authorization configuration from a YAML file or creates a default configuration if the file is missing or invalid.
        
        Parses tool access settings into internal mappings and builds agent-to-tool associations for permission checks.
        """
        try:
    ...
    def _create_default_config(self):
        """
        Creates and writes a default tool authorization configuration file if none exists.
        
        Initializes the system with predefined tool permissions and updates internal mappings.
        """
        default_config = {
            'tools': [
                {
    ...
    def _build_agent_tool_map(self):
        """
        Constructs a mapping from agent roles to the set of tool IDs they are permitted to access.
        
        Agent roles with a wildcard ('*') are excluded from this mapping, as they represent permissions for all agents.
        """
    ...
    def can_agent_use_tool(self, agent_role: str, tool_id: str) -> Tuple[bool, Optional[str]]:
        ...
        Checks if the tool exists, verifies the agent's permissions (including wildcard access), and enforces any configured rate limits. Returns a tuple indicating whether access is allowed and, if denied, the reason.
    ...
    def _check_rate_limits(self, tool_id: str) -> Tuple[bool, Optional[str]]:
        """
        Determines whether a tool has exceeded its configured rate limits.
        
        Checks the tool's usage history to enforce per-minute and per-hour call limits. Returns a tuple indicating if the tool is currently rate-limited and, if so, the reason.
         
        Args:
            tool_id: The identifier of the tool to check.
        
        Returns:
            A tuple (rate_limited, reason), where rate_limited is True if the tool has exceeded its rate limits, and reason provides details if rate-limited.
        """
        access = self.tool_access_map[tool_id]
    ...
    def register_tool_usage(self, tool_id: str, agent_id: str, success: bool, 
        ...
        """
                           Records a tool usage event with metadata including agent, parameters, and execution time.
                           
                           Adds a new usage record to the history, maintaining a maximum of 10,000 recent events.
                           """
        usage = ToolUsage(
            tool_id=tool_id,
            agent_id=agent_id,
    ...
    def get_agent_allowed_tools(self, agent_role: str) -> List[str]:
        ...
        Includes tools explicitly assigned to the agent role as well as those available to all agents via wildcard permissions.
        """
    ...
    def verify_tools_for_agent(self, agent_role: str, required_tools: List[str]) -> Tuple[bool, List[str]]:
        """
        Checks if an agent role has access to all specified required tools.
        
        Args:
            agent_role: The role of the agent to check permissions for.
            required_tools: List of tool IDs that the agent must have access to.
        
        Returns:
            A tuple where the first element is True if the agent has access to all required tools,
            and the second element is a list of tool IDs the agent lacks access to.
        """

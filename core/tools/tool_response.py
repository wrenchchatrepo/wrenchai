...
def format_success_response(data: Any = None, meta: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Constructs a standardized success response dictionary for tool operations.
    
    Args:
        data: Optional result data to include in the response.
        meta: Optional metadata to attach to the response.
    
    Returns:
        A dictionary representing a successful tool response with data, metadata, and timestamp.
    """
...
def format_error_response(error_message: str, meta: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    Returns:
        A dictionary representing the error response in the standardized format.
    """
    response = ToolResponse[
...
def standardize_legacy_response(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    transforms legacy success or error responses into the standard format, preserving
    the original response in metadata and marking it as converted.
     
    Args:
        response: A legacy tool response dictionary.
    
    Returns:
        A standardized response dictionary compatible with the ToolResponse schema.
    """
